...
[InspectableList("disabled", "visible", "itemRenderer", "inspectableScrollBar", "rowHeight", "columnWidth", "direction", "inspectableRendererInstanceName", "externalColumnCount", "margin", "enableInitCallback", "soundMap", "wrapOutOfSet", "wrapping")]
...
// Public Methods:
	...
	{		
		...
		do
		{
			...
			var dpLength:Number = dataProvider.length;
			if (nextIndex >= dpLength)
			{
				if (wrapping == "stick")
				{
					// "stick" will not move to the last renderer if we're already in the last row or column. It will simply stick there.					
					if ( currentSet == Math.floor(dpLength / totalColumns) && _direction == "vertical" )
					{
						nextIndex = _selectedIndex;
					}
					else if ( currentSet == Math.floor(dpLength / totalRows) && _direction == "horizontal" )
					{
						nextIndex = _selectedIndex;
					}
					else 
					{
						// Find the last selectable renderer.
						nextIndex = dpLength;
						nextDelta = -1;
					}
				}
			...
			else if (nextIndex < 0)
			{
				if (wrapping == "stick")
				{					
					// "stick" will not move to the first renderer if we're already in the first row or column. It will simply stick there.
					if ( currentSet == 0 )
					{
						nextIndex = _selectedIndex;
					}
					...
					{
						// Find the first selectable renderer.
						nextIndex = -1;
						nextDelta = 1;
					}
				} 
...
// Private Methods:
	...
	private function drawLayout(rendererWidth:Number, rendererHeight:Number):Void {
		...
		for (var i:Number = 0; i < arrayLength; i++) {
			...
			r.setSize(rendererWidth, rendererHeight);
		}
		drawScrollBar();
	}
	...
	private function populateData(data:Array):Void {
		...
		for (var i:Number=0; i < totalColumns*totalRows; i++) {
			...
			renderer.setListData(index, itemToLabel(data[i]), _selectedIndex == index); //LM: Consider passing renderer position also. (Supports animation)
			...
			bHasASelectableItem = bHasASelectableItem || ( data[i] != undefined && data[i].unselectable != true );			
		}
	}
