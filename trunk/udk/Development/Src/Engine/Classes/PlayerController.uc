...
`endif
	...
	if (IsBestHostPeer(PlayerReplicationInfo.UniqueId))
	{
		...
		{
			// End the game session so it can be migrated
			//OnlineSub.GameInterface.AddEndOnlineGameCompleteDelegate(OnEndGameForHostMigrate);
			//OnlineSub.GameInterface.EndOnlineGame(PlayerReplicationInfo.SessionName);

			// No need to end game session on migration
			OnEndGameForHostMigrate(PlayerReplicationInfo.SessionName,true);
		}
		else
		{
...
/**
 * Delegate called once the end of an online game completes so can migrate as host.
 *
 * @param SessionName the name of the session being ended
 * @param bWasSuccessful whether the end completed ok or not
 */
function OnEndGameForHostMigrate(name SessionName, bool bWasSuccessful)
{
	// Clear delegate reference
	OnlineSub.GameInterface.ClearEndOnlineGameCompleteDelegate(OnEndGameForHostMigrate);
	// Remove all non-peers from session
	RemoveMissingPeersFromSession(SessionName);
}
 ...
 */
function RemoveMissingPeersFromSession(name SessionName)
{
		...
		OnlineSub.GameInterface != None)
	{
		...
		if (RegisteredPlayers.Length > 0)
		{
			...
			OnlineSub.GameInterface.AddUnregisterPlayerCompleteDelegate(OnUnregisterPlayerCompleteForMigrate);
		...
		else
		{
			...
			OnlineSub.GameInterface.MigrateOnlineGame(LP.ControllerId,SessionName);
		}
	}
}
 ...
 */
function OnUnregisterPlayerCompleteForMigrate(name SessionName,UniqueNetId PlayerId,bool bWasSuccessful)
{
	// Clear the delegate so another unregister can occur
	OnlineSub.GameInterface.ClearUnregisterPlayerCompleteDelegate(OnUnregisterPlayerCompleteForMigrate);
	// Continue unregistering players until none are left
	RemoveMissingPeersFromSession(SessionName);
}
...
event PeerReceivedMigratedSession(UniqueNetId FromPeerNetId, name SessionName, class<OnlineGameSearch> SearchClass, byte PlatformSpecificInfo[80])
{
		...
		OnlineSub.GameInterface != None)
	{
		...
		if (OnlineSub.GameInterface.BindPlatformSpecificSessionToSearch(LP.ControllerId,MigratedSearchToJoin,PlatformSpecificInfo))
		{
			...
			{
				// End current session if it exists iot migrate to new one
 				//OnlineSub.GameInterface.AddEndOnlineGameCompleteDelegate(OnEndGameForJoinMigrate);
 				//OnlineSub.GameInterface.EndOnlineGame(SessionName);

				// No need to end game session on migration
				OnEndGameForJoinMigrate(SessionName,true);
			}
			else
			{
				// Join migrated session without ending current one
				OnEndGameForJoinMigrate(SessionName,true);
			}
		}
		else
...
/**
 * Delegate called once the end of an online game completes so can join a migrated one.
 *
 * @param SessionName the name of the session being ended
 * @param bWasSuccessful whether the end completed ok or not
 */
function OnEndGameForJoinMigrate(name SessionName, bool bWasSuccessful)
{
	...
	// Clear the delegate for ending game
	OnlineSub.GameInterface.ClearEndOnlineGameCompleteDelegate(OnEndGameForJoinMigrate);
	...
	OnlineSub.GameInterface.JoinMigratedOnlineGame(LocalPlayer(Player).ControllerId,SessionName,SessionToJoin);
}
