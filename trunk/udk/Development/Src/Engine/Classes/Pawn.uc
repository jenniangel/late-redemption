 ...
 */
native event bool IsValidEnemyTargetFor(const PlayerReplicationInfo PRI, bool bNoPRIisEnemy);
...
simulated function OnTeleport(SeqAct_Teleport Action)
{
	...
	if( !HandleTeleport( objVars, Action.bUpdateRotation, Action.bCheckOverlap ) )
	{
		`warn( "failed to handle teleport kismet action properly"@Action );		
	}
}

simulated function bool HandleTeleport( array<Object> DestList, bool bUpdateRotation, bool bCheckOverlap, optional float TeleportDistance, optional array<Volume> TeleportVolumes )
{
	local int idx;
	...
	for (idx = 0; idx < DestList.Length; idx++)
	{
