...
public:
	 ...
	 */
	BITFIELD* GetInterpBoolPropertyRef( FName InName );
...
simulated function bool EffectIsRelevant(vector SpawnLocation, bool bForceDedicated, optional float VisibleCullDistance=5000.0, optional float HiddenCullDistance=350.0 )
{
	...
	local float   BoundsRadius, BoundsHeight;
	local Vector X, Y, Z;
	local array<Vector> TestLocations;
	local int Idx;
	local float InViewThreshold;
	...
	if( `TimeSince( LastRenderTime ) < 0.3f )
	{
		`debugEffectIsRelevant( "EffectIsRelevant FAILED - SpawnLocation == Location", ( WorldInfo.TimeSeconds - LastRenderTime < 0.3 ) );
	...
	GetBoundingCylinder( BoundsRadius, BoundsHeight );
	ForEach LocalPlayerControllers(class'PlayerController', P)
	{
		P.GetPlayerViewPoint(CameraLoc, CameraRot);
		...
		{
			if( FastTrace(SpawnLocation, CameraLoc) )
			{
		...
		if( !bIsInViewFrustrum )
		{
			// Check the camera aligned bounding box
			GetAxes( CameraRot, X, Y, Z );
			TestLocations[TestLocations.Length] = SpawnLocation + Y * BoundsRadius + Z * BoundsHeight;
			TestLocations[TestLocations.Length] = SpawnLocation - Y * BoundsRadius + Z * BoundsHeight;
			TestLocations[TestLocations.Length] = SpawnLocation + Y * BoundsRadius - Z * BoundsHeight;
			TestLocations[TestLocations.Length] = SpawnLocation - Y * BoundsRadius - Z * BoundsHeight;

			for( Idx = 0; Idx < TestLocations.Length; Idx++ )
			{
				if( (Normal(TestLocations[Idx] - CameraLoc) Dot vector(CameraRot)) > InViewThreshold )
				{
					if( FastTrace(TestLocations[Idx], CameraLoc) )
					{
						bIsInViewFrustrum = TRUE;
						break;
					}
