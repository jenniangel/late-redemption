...
/** The singleton interface that manages ingame ads */
var const transient InGameAdManager AdManager;

/** The name of the class to use for the Ad manager */
...
/** Returns the AdManager object. This could be NULL depending on the platform */
native static final noexport function InGameAdManager GetAdManager();
...
cpptext
{
	...
	static UInGameAdManager* GetAdManager(void)
	{
		UGameEngine* GameEngine = Cast<UGameEngine>(GEngine);
		if (GameEngine)
		{
			return GameEngine->AdManager;
		}
		return NULL;
	}
