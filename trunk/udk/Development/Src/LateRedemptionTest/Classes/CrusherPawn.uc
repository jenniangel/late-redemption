...
var CrusherController myController;         // Crusher IA Controller
...
var bool crusherAwaking;                    // First see or first shot
...
// just by pressing F4 over a CrusherPawn monster.
//------------------------------------------------------------------
...
simulated function PostBeginPlay()
{
   ...
   {
      myController = Spawn(class'CrusherController', self);
...
//------------------------------------------------------------------
function SetAttacking(bool atacar, bool awaking)
{
	crusherAwaking = awaking;
	AttAcking = atacar;
	if (awaking)
	{
       self.PlaySound(crusherAwake, false, true);
	}
}
...
// the notification that the Crusher has been hit requesting to
// increase its speed velocity.
// Before changing the speed, the Crusher must check whether the
// new requested value is within acceptable limits.
// On the opposite direction, when revenge timer runs out, this 
// function is internally called in order to decrease the Crusher
// speed, changing in this way its Attack sub-state
// It means that in case the ChangePawnSpeed succeeds in changing the
// Crusher speed, it is going from Attack to Revenge or from Revenge
// to Insane states.
...
//------------------------------------------------------------------
// This event is notified whenever the Crusher is hit. It has been
...
// Crusher speed will be carried out by calling the ChangePawnSpeed
