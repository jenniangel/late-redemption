...
struct AchievementData
{
	...
	var bool			bDoUnlock;		// Wether or not the achievement manager should handle unlocks for this acheivement
								//	(achievements can be setup so OnlineSubsystemSteamworks handles this)
	var bool			bDoProgress;		// Wether or not the achievement manager should handle progress toasts for this
 ...
 * @param bProgressEnabled	Wether or not progress toasts are enabled for this achievement
 ...
 * @param bUnlockEnabled	Wether or not unlocking is enabled for this achievement
 * @return			Returns True if successful, False otherwise
 */
	...
	if (OnlineSub != none)
	{
		...
		if (AchievementStatsRead != none)
		{
			...
			if (!OnlineSub.StatsInterface.ReadOnlineStats(UIDList, AchievementStatsRead))
			{
				...
				{
					ReadStatsCount++;
					SetTimer(1.0, True, 'ReadAchievementStats', Self);
				}
				else
				{
