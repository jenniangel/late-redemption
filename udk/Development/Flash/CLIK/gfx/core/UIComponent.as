...
// Initialization:
	...
	private function onLoadImpl():Void {        
		...
		if (_focused != 0 && Selection.getFocusBitmask(this) == 0) {
			var numFocusGroups:Number = Selection.numFocusGroups;
			for (var i:Number = 0; i < numFocusGroups; i++) {
				...
				if (focused) {
					...
					var numControllers:Number = capabilities["numControllers"];
...
// Public Methods:
	 ...
	 */
	 public static function createInstance(context:MovieClip, symbol:String, name:String, depth:Number, initObj:Object):MovieClip {
		 ...
		 if (mc.invalidate) { mc.invalidate(); }
		 return mc;
	 }
		...
		for (var i:Number = 0; i < numFocusGroups; i++) {
			...
			if (focused) {
				...
				for (var j:Number = 0; j < numControllers; j++) {
					...
					if (!controllerValue && Selection.getFocus(j) == this) {
						Selection.setFocus(null, j);
					} else if (controllerValue && Selection.getFocus(j) != this) {
