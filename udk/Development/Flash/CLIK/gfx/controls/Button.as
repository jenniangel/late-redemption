 ...
 * 	Buttons are the foundation component of the CLIK framework and are used anywhere a clickable interface control is required. The default Button class (gfx.controls.Button) supports a textField to display a label, and states to visually indicate user interaction. Buttons can be used on their own, or as part of a composite component, such as ScrollBar arrows or the Slider thumb. Most interactive components that are click-activated compose or extend Button.
	...
	<li><i>enableInitCallback</i>: If set to true, when the widget is created, the owning movie will have "WidgetInitialized" called on it.</li></ul>
...
[InspectableList("disabled", "disableFocus", "visible", "toggle", "labelID", "disableConstraints", "bindingEnabled", "enableInitCallback", "autoSize", "soundMap")]
...
// Public Methods:
	...
	/**
	 * Determines if the button will scale to fit the text that it contains. Setting the autoSize property to {@code autoSize=false} will leave its current size unchanged.
	 */
		...
		switch(details.navEquivalent) {
			case NavigationCode.ENTER:
				if (details.value == "keyDown") { // A more generic solution may be required for Button
					if (!pressedByKeyboard) { handlePress(); }
				} else {
					handleRelease();
				}
...
// Private Methods:
	...
	private function alignForAutoSize():Void {
		...
		textField._width = newWidth;
		__width = _width = newWidth;
	...
	private function changeFocus():Void {
		...
		if (focusIndicator != null) {
			if (focusIndicator._totalframes == 1) { focusIndicator._visible = _focused; } // Do a simple show/hide on single-frame focus indicators.
			else { focusIndicator.gotoAndPlay(_focused ? "show" : "hide"); }
	...
	private function handleMouseRollOut(mouseIndex:Number):Void {
		...
		dispatchEventAndSound({type:"rollOut", mouseIndex:mouseIndex});
	}
	...
	private function handleMousePress(mouseIndex:Number, button:Number):Void {
		...
		dispatchEventAndSound({type:"press", mouseIndex:mouseIndex, button:button});		
		if (autoRepeat) {
	...
	private function handlePress():Void {
		...
		dispatchEventAndSound({type:"press"});
	}
	...
	private function handleMouseRelease(mouseIndex:Number, button:Number):Void {
		...
		if (doubleClickEnabled) {
			if (doubleClickInterval == null) {
				...
				doubleClickExpired();
				dispatchEventAndSound({type:"doubleClick", mouseIndex:mouseIndex, button:button});
				setState("release");
				return;
			}
		}
		...
		setState("release");
		handleClick(mouseIndex, button); // State changes in the handleClick will override those in the setState or focus change (above)
	}
	// #state invoked by keyboard
	private function handleRelease():Void {
		if (_disabled) { return; }
		setState(focusIndicator == null ? "release" : "kb_release");
		handleClick(null); // State changes in handleClick will override those in the setState (above)
		pressedByKeyboard = false;
	}
	 ...
	 */
	private function handleClick(mouseIndex:Number, button:Number):Void {
		if (toggle) { selected = !_selected; } // Will cause a state change.
		dispatchEventAndSound({type:"click", mouseIndex:mouseIndex, button:button, invokedButton:this});		
	}
	...
	private function handleDragOver(mouseIndex:Number):Void {
		...
		dispatchEvent({type:"dragOver", mouseIndex:mouseIndex});
	}
	private function handleDragOut(mouseIndex:Number):Void {
		...
		dispatchEvent({type:"dragOut", mouseIndex:mouseIndex});
	}	
	...
	private function beginButtonRepeat(mouseIndex:Number, button:Number):Void {
		clearRepeatInterval();
	...
	private function handleButtonRepeat(mouseIndex:Number, button:Number):Void {
		dispatchEventAndSound({type:"click", mouseIndex:mouseIndex, button:button});
	}
