...
{
	if (bNetDirty)
		bHasFlag, bHolding, Squad, OrdersIndex, CharClassInfo, bIsFemale;
}
...
function UpdateEventStatAchievements(name StatName)
{
	...
	if (PC != none)
	{
		if (StatName == 'KILLS_SHOCKRIFLE' || StatName == 'KILLS_SHOCKCOMBO')
			PC.ClientUpdateAchievement(EUTA_WEAPON_DontTaseMeBro, 1);
		else if (StatName == 'KILLS_LINKGUN')
			PC.ClientUpdateAchievement(EUTA_WEAPON_StrongestLink, 1);
		else if (StatName == 'KILLS_ROCKETLAUNCHER')
