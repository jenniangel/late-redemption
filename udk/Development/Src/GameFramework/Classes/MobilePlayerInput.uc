...
cpptext
{
	...
	/**
	 * Perform any per-processing on a touch event.  Note this function assumes that the touches structure is already
	 * filled out.
	 *
	 * @param TouchIndex		The Index of the touch we are processing
	 * @param Handle			the id of the touch
	 */
	void PreProcessTouch(INT TouchIndex, UINT Handle);
	 ...
	 *
	 * @param Attitude		The inclination of the device
	 * @param RotationRate	How fast the attitude is changing
	 * @param Gravity		Describes the current gravity of the device
	 * @param Acceleration  Describes the acceleration of the device
	 */
	void ProcessMotion(const FVector& Attitude, const FVector& RotationRate, const FVector& Gravity, const FVector& Acceleration);
	 ...
	 */
	void ProcessAccelerometer(const FVector& Acceleration);
	 ...
	 */
	void ProcessGyro(const FVector& RotationRate);
	 ...
	 */
	static void ModifyVectorByOrientation(FVector& Vec, EUIOrientation Orientation)
	{
		switch (Orientation)
		{
			case UI_Portait:
	 ...
	 */
 	void NativeInitializeZone(UMobileInputZone* Zone, const FVector2D& ViewportSize);
};
